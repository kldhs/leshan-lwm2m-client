...
public abstract class DataSerializer {
	public static final Logger LOG = LoggerFactory.getLogger(UDPConnector.class);
	/**
	...
	public static void serializeOptionsAndPayload(DatagramWriter writer, final OptionSet optionSet,
		...
		for (Option option : optionSet.asSortedList()) {
			...
			a = a+"optionDeltaNibble: "+optionDeltaNibble;
			...
			a =a+ ", optionLengthNibble: "+optionLengthNibble;

			// write extended option delta field (0 - 2 bytes)
			if (optionDeltaNibble == 13) {
				writer.write(optionDelta - 13, Byte.SIZE);
				a =a+ ", extended optionDelta-13: "+(optionDelta-13);
			} else if (optionDeltaNibble == 14) {
				writer.write(optionDelta - 269, 2 * Byte.SIZE);
				a =a+ ", extended optionDelta-269: "+(optionDelta-269);
			}

			// write extended option length field (0 - 2 bytes)
			if (optionLengthNibble == 13) {
				writer.write(optionLength - 13, Byte.SIZE);
				a =a+ ", extended optionLength-13: "+(optionLength-13);
			} else if (optionLengthNibble == 14) {
				writer.write(optionLength - 269, 2 * Byte.SIZE);
				a =a+ ", extended optionLength-269: "+(optionLength-269);
			}

			// write option value
			writer.writeBytes(value);
			a=a+", optionNumber: "+optionNumber;
			a=a+", value: "+option.toString();
		...
		if (payload != null && payload.length > 0) {
            ...
            LOG.error("编码---->> "+"拼接payload, payload: {}",payload);
		}
	}
