 ...
 *                                                     as UPDATE instance
 *******************************************************************************/
package org.eclipse.leshan.client.demo.clientcf.object;
...
import org.eclipse.leshan.client.demo.clientcf.LwM2mClientCoapResource;
...
public class ObjectResource extends LwM2mClientCoapResource implements ObjectListener {
    ...
    public void handleGET(CoapExchange exchange) {
        LOGGER.error("处理请求---->> "+"ObjectResource处理 get 请求，getRequest : {}", exchange.advanced().getRequest());
        ...
        if (exchange.getRequestOptions().getAccept() == MediaTypeRegistry.APPLICATION_LINK_FORMAT) {
            ...
            if (exchange.getRequestOptions().hasObserve()) {
                ...
                if (response.getCode() == org.eclipse.leshan.core.ResponseCode.CONTENT) {
                    ...
                    LOGGER.error("发送---->> "+"leshan ObjectResource发送响应，content : {}",content );
    ...
    public void handlePUT(CoapExchange coapExchange) {
        LOGGER.error("处理请求---->> "+"ObjectResource处理 put 请求，getRequest : {}", coapExchange.advanced().getRequest());
    ...
    public void handlePOST(CoapExchange exchange) {
        LOGGER.error("处理请求---->> "+"ObjectResource处理 post 请求，getRequest : {}", exchange.advanced().getRequest());
        ServerIdentity identity = getServerOrRejectRequest(exchange);
        if (identity == null)
            return;
    ...
    public void handleDELETE(CoapExchange coapExchange) {
        LOGGER.error("处理请求---->> "+"ObjectResource处理 detele 请求，getRequest : {}", coapExchange.advanced().getRequest());
    ...
    public void resourceChanged(LwM2mObjectEnabler object, int instanceId, int... resourceIds) {
        ...
        // exits.
        LOG.error("资源更新---->> "+"调用handleRequest方法，resourceIds : {}", Arrays.stream(resourceIds).mapToObj(a -> String.valueOf(a)).collect(Collectors.joining(",")));
