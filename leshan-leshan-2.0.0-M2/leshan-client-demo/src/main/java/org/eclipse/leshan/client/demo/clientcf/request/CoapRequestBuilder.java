...
public class CoapRequestBuilder implements UplinkRequestVisitor {
    ...
    public void visit(RegisterRequest request) {
        ...
        logger.error("创建请求---->>"+"添加OptionSet属性：\"Content-Format\":\"application/link-format\"");
        coapRequest.getOptions().addUriPath("rd");
        logger.error("创建请求---->>"+"添加OptionSet属性：\"Uri-Path\":\"rd\"");
        HashMap<String, String> attributes = new HashMap<>();
        attributes.putAll(request.getAdditionalAttributes());
        ...
        if (objectLinks != null)
            logger.error("创建请求---->>"+"添加payload : "+ JSONUtil.toJsonStr(Link.serialize(objectLinks)));
