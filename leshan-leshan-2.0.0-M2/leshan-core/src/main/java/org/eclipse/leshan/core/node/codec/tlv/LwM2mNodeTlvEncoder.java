    ...
    private static class InternalEncoder implements LwM2mNodeVisitor {
        ...
        private byte[] encodeTlvValue(Object value, Type type, LwM2mPath path) {
            ...
            try {
                ...
                switch (type) {
                case STRING:
                    bytes = TlvEncoder.encodeString((String) value);
                    LOG.error("编码---->> "+"原始值 : {},类型 : {},编码后的byte[] : {}",value,type,bytes);
                    return bytes;
                case INTEGER:
                case UNSIGNED_INTEGER:
                    bytes = TlvEncoder.encodeInteger((Number) value);
                    LOG.error("编码---->> "+"原始值 : {},类型 : {},编码后的byte[] : {}",value,type,bytes);
                    return bytes;
                case FLOAT:
                    bytes = TlvEncoder.encodeFloat((Number) value);
                    LOG.error("编码---->> "+"原始值 : {},类型 : {},编码后的byte[] : {}",value,type,bytes);
                    return bytes;
                case BOOLEAN:
                    bytes = TlvEncoder.encodeBoolean((Boolean) value);
                    LOG.error("编码---->> "+"原始值 : {},类型 : {},编码后的byte[] : {}",value,type,bytes);
                    return bytes;
                case TIME:
                    bytes = TlvEncoder.encodeDate((Date) value);
                    LOG.error("编码---->> "+"原始值 : {},类型 : {},编码后的byte[] : {}",value,type,bytes);
                    return bytes;
                case OPAQUE:
                    bytes = (byte[]) value;
                    ...
                    return bytes;
                case OBJLNK:
                    bytes = TlvEncoder.encodeObjlnk((ObjectLink) value);
                    LOG.error("编码---->> "+"原始值 : {},类型 : {},编码后的byte[] : {}",value,type,bytes);
                    return bytes;
                default:
